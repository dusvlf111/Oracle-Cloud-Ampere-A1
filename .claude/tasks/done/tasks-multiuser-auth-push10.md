...
- [x] 10.0 웹 권한 분리 UI (Push 10)
    ...
    - [x] 10.3 유저 관리 페이지 (admin) — `pages/users` + `features/user-approve`: 목록 테이블 (pending 상단 정렬 + 뱃지), 행별 승인/거부/비활성/활성 버튼 (거부는 확인 모달), 액션 후 캐시 무효화. 모바일 카드 전환 (기존 패턴)
        - [x] 10.3.T1 vitest 테스트 작성 — 목록 렌더, 승인 클릭→PATCH 요청+목록 갱신, 거부 확인 모달, 비활성 (MSW)
        - [x] 10.3.T2 `pnpm --filter web vitest run src/pages/users src/features/user-approve` 실행 및 검증
    - [x] 10.4 사이드바 role 분기 + 라우트 — admin 에게만 "유저 관리" 메뉴 노출 (me.role 기반), user 가 `/users` 직접 접근 시 서버 403/404 → 안내 화면, `app/(protected)/users/page.tsx` 라우트
        - [x] 10.4.T1 vitest 테스트 작성 — admin/user 별 사이드바 메뉴 분기, 비 admin 접근 안내
        - [x] 10.4.T2 `pnpm --filter web test` 전체 + `pnpm --filter web lint` (FSD) + 커버리지 50%+ 게이트 실행 및 검증
