...
모든 구현 작업 뒤에 자동 추가:
- `[N].T1` 테스트 코드 작성 (pytest)
- `[N].T2` 테스트 실행 및 검증 (`pytest -q` 또는 `python -m pytest`)
...
- [ ] 1.0 상위 작업 (Push 범위)
    - [ ] 1.1 하위 작업 (커밋 단위)
        - [ ] 1.1.T1 pytest 테스트 코드 작성
        - [ ] 1.1.T2 `pytest -q tests/test_foo.py` 실행 및 검증
    - [ ] 1.2 하위 작업 (커밋 단위)
        - [ ] 1.2.T1 pytest 테스트 코드 작성
        - [ ] 1.2.T2 테스트 실행 및 검증
```
...
- Python 작업은 타입 힌트와 docstring을 기본으로 권장
- 완료 후 생성된 파일 목록만 간단히 보고

## 참고 자료

- 행동 가이드: [behavioral.md](../../behavioral.md)
